...
@IonicPage()
@Component({
	selector: 'page-waivers',
	templateUrl: 'list-waivers.html'
})
export class ListWaiversPage {
	...
	loadWaivers() {
		...
		this.translate.get(['LOADING_MESSAGE']).subscribe((value) => {
			...
			let seq = this.transaction.WaiverList();

			seq.subscribe((res: any) => {
				if (res.Waivers.length != 0) {
					this.currentItems = res.Waivers;
				}				
